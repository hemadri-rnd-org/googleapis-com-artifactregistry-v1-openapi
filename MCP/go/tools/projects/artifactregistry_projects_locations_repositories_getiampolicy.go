...
func Artifactregistry_projects_locations_repositories_getiampolicyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateArtifactregistry_projects_locations_repositories_getiampolicyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_resource_getIamPolicy",
		mcp.WithDescription("Gets the IAM policy for a given resource."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Artifactregistry_projects_locations_repositories_getiampolicyHandler(cfg),
	}
}
