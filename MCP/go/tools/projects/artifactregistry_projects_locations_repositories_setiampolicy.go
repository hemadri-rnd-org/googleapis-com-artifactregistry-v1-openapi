...
func Artifactregistry_projects_locations_repositories_setiampolicyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateArtifactregistry_projects_locations_repositories_setiampolicyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v1_resource_setIamPolicy",
		mcp.WithDescription("Updates the IAM policy for a given resource."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Artifactregistry_projects_locations_repositories_setiampolicyHandler(cfg),
	}
}
