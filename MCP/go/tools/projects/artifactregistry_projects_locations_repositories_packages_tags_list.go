...
func Artifactregistry_projects_locations_repositories_packages_tags_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/v1/%s/tags%s", cfg.BaseURL, parent, queryString)
...
func CreateArtifactregistry_projects_locations_repositories_packages_tags_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_v1_parent_tags",
		mcp.WithDescription("Lists tags."),
		mcp.WithString("parent", mcp.Required(), mcp.Description("The name of the parent package whose tags will be listed. For example: `projects/p1/locations/us-central1/repositories/repo1/packages/pkg1`.")),
		...
		mcp.WithNumber("pageSize", mcp.Description("The maximum number of tags to return. Maximum page size is 10,000.")),
		mcp.WithString("pageToken", mcp.Description("The next_page_token value returned from a previous list request, if any.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Artifactregistry_projects_locations_repositories_packages_tags_listHandler(cfg),
	}
}
