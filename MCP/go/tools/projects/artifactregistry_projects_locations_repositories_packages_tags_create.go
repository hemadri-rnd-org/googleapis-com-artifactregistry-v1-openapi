...
func Artifactregistry_projects_locations_repositories_packages_tags_createHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["tagId"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("tagId=%v", val))
		}
...
func CreateArtifactregistry_projects_locations_repositories_packages_tags_createTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_v1_parent_tags",
		mcp.WithDescription("Creates a tag."),
		mcp.WithString("parent", mcp.Required(), mcp.Description("The name of the parent resource where the tag will be created.")),
		mcp.WithString("tagId", mcp.Description("The tag id to use for this repository.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Artifactregistry_projects_locations_repositories_packages_tags_createHandler(cfg),
	}
}
